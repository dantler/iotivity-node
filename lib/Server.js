...
_.extend( Server.prototype, {
	...
	_createEntityHandler: function( resource ) {
		return _.bind( function( flag, request ) {
			var thePayload, queryInfo, eventName, observeFlag, queryString;
			...
			thePayload = request.payload ? payload.repPayloadToObject( request.payload ) : null;
			if ( thePayload instanceof Error ) {
				this.emit( "error", _.extend( thePayload, errorExtension ) );
				return csdk.OCEntityHandlerResult.OC_EH_ERROR;
			}
			...
			eventName =
				...
				request.method === csdk.OCMethod.OC_REST_POST ? "update" :
				request.method === csdk.OCMethod.OC_REST_PUT ? "create" :
			...
			this.emit( eventName, Request( _.extend( {
				...
				thePayload !== null ? { data: thePayload } : {},
