...
var payloadUtils = {
	initToCreateRequestPayload: function( init ) {
		return _.extend( payloadUtils.objectToRepPayload( {
	...
	bitmapToInit: function( p ) {
		return {
			secure: p.sec,
			discoverable: !!( p.bm & csdk.OCResourceProperty.OC_DISCOVERABLE ),
			observable: !!( p.bm & csdk.OCResourceProperty.OC_OBSERVABLE ),
			slow: !!( p.bm & csdk.OCResourceProperty.OC_SLOW ),
			active: !!( p.bm & csdk.OCResourceProperty.OC_ACTIVE )
		};
	},
	resourceFromRepresentation: function( payload, deviceId ) {
		...
		}, payloadUtils.bitmapToInit( object.p ) );

		return payloadUtils.validateResource( resource ) ?
			resource : _.extend( new Error( "Invalid resource representation from device" ), {
				deviceId: deviceId,
				payload: payload
			} );
	},
